...
class MotionControllerTeleoperation {
 public:
  ...
  inline void resetToReadyPose() {
    // The ready pose is not generated by generateInitialGripperPose(), so we
    // approximate the view angle from the target position's XY angle.
 ...
 private:
  ...
  // [radian] the net yaw applied during the current position control session;
  //  folded into control_view_angle_ when the session ends (safety button
  //  released)
  double last_position_yaw_;
