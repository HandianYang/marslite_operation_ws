...
class MotionControllerTeleoperation {
 public:
  ...
  // Exponential smoothing factor for the published pose. Each tick:
  //   published = prev + alpha * (desired - prev)
  // This prevents the fzi forward-dynamics solver from seeing pose
  // discontinuities (which excite its internal velocity integrator), while
  // tracking fast movements with only a small proportional lag. On stop, the
  // residual error decays exponentially (natural deceleration) rather than
  // linearly (constant-speed crawl that looks like swinging/dragging).
  //   alpha = 0.7 @ 50 Hz ≈ 9 Hz bandwidth, ~60 ms settling from a 30 mm jump.
  // Tune range: 0.5 (smoother, more lag) .. 0.9 (snappier, less filtering).
  static inline constexpr double kPoseSmoothingAlpha = 0.7;
  // Hard safety cap applied AFTER exponential smoothing. These are generous
  // enough that normal teleop never reaches them — they only activate during
  // a genuine runaway (e.g. the fzi solver's velocity integrator is
  // pre-loaded from a previous session). Acts as a second safety net.
  // [m/tick] 0.015 m @ 50 Hz = 0.75 m/s. Normal teleop peak ≈ 0.3 m/s.
  static inline constexpr double kMaxPositionStepPerTick = 0.015;
  // [rad/tick] 0.10 rad @ 50 Hz = 5 rad/s. Normal teleop peak ≈ 2 rad/s.
  static inline constexpr double kMaxOrientationStepPerTick = 0.10;
  ...
  /**
   * @brief Main loop for teleoperation control.
   */
  void run();

  /**
   * @brief Log the gripper's current cylindrical pose (radius, yaw, height)
   *   and the operator's control_view_angle_ for debugging.
   */
  void printCylindricalPose() const;
 ...
 private:
  ...
  CylindricalPoint scaleCylindricalPositionDifference(const CylindricalPoint& pos_diff);
  void applyCylindricalPositionDifference(const CylindricalPoint& scaled_diff);
  Eigen::Vector3d getPositionWithRadiusAndHeightApplied(const CylindricalPoint& scaled_diff);
  ...
  void publishDesiredGripperPose();
  ...
  // ROS messages
  geometry_msgs::Point shoulder_position_;  // origin of the user space
  ...
  // The last pose actually sent to the cartesian controller. Rate-limited
  //  and quaternion-sign-consistent to prevent exciting the fzi forward
  //  dynamics solver's internal velocity integrator.
  geometry_msgs::PoseStamped last_published_pose_;
